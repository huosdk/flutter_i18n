...
//https://dt9qhxywftqdz.cloudfront.net/www.jetbrains.com/intellij-repository/releases/com/jetbrains/intellij/idea/ideaIC/182.5107.41/ideaIC-182.5107.41-sources.jar

class I18nFileGenerator(private val project: Project) {
    private final var defaultLanguage = "zh";
    ...
    fun generate() {
        ...
        if (files.isEmpty()) {
//            files.add(createFileForLang(defaultLanguage))
            return;
        }
        ...
        appendSClass(data[defaultLanguage]!!, builder)

        val className = "\$" + defaultLanguage
        data.keys.forEach {
            if (it == defaultLanguage) builder.append("class $className extends S {\n  const $className();\n}\n\n")
    ...
    private fun appendSClass(en: HashMap<String, String>, builder: StringBuilder) {
        ...
        //插入多语言代码中引用的枚举类 start
        builder.append("\n\n")
        builder.append("enum AppStrings {\n")
        ids.forEach { appendStringEnum(it, en[it]!!, builder, false) }
        builder.append("}\n\n")
        //插入多语言代码中引用的枚举类 end

        builder.append(sClassHeader)

        //插入根据枚举常量获取值的方法 start
        builder.append("  String value(AppStrings name) {\n")
        builder.append("    switch (name) {\n")
        ids.forEach { appendValueMethod(it, en[it]!!, builder, false) }
        builder.append("    }\n")
        builder.append("    return \"\";\n")
        builder.append("  }\n")
//        builder.append(
//            """  String value(AppStrings name) {
//    switch (name) {"""
//        )
//        ids.forEach { appendValueMethod(it, en[it]!!, builder, false) }
//        builder.append(
...
//""")
     ...
     * 添加字符串枚举常量
     */
    internal fun appendStringEnum(id: String, value: String, builder: StringBuilder, isOverride: Boolean = true) {
        builder.append("  $id,\n")
    }
     ...
     * 添加字符串值获取方法
     */
    internal fun appendValueMethod(id: String, value: String, builder: StringBuilder, isOverride: Boolean = true) {
        builder.append(
            "      case AppStrings.$id:\n" +
                    "        return $id;\n"
        )
...
"""
  ...
  /// 主动更新
  static void update(BuildContext context) => S.current=Localizations.of<S>(context, S);
